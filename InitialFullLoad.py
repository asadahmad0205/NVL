...
query = "SELECT distinct table_name FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'db_1' and table_type='BASE TABLE'   and table_name not in  ('sharepoint_updates_timestamps','audit','deal_rate_floors','forbury_calculator');"
...
# Loop through each record and write to a separate JSON file
    for i, record in enumerate(records):
        human_readable_date = record['created_at']
        output_dir =source_table_name+'/'+human_readable_date.strftime('%Y')+'/'+human_readable_date.strftime('%m')+'/'+human_readable_date.strftime('%d')+'/WriteRowsEvent'
        os.makedirs(output_dir, exist_ok=True)

        filext=str(human_readable_date.strftime('%Y%m%d%H%M%S'))
        file_name=source_table_name+str(record['id'])+filext+'.json'
        file_path = output_dir+'/'+file_name
        record['RECORD_GENERATED_AT']=human_readable_date
        with open(file_path, 'w') as json_file:
            json.dump(record, json_file, indent=20,default=str)
